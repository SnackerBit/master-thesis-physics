...
\begin{document}
	...
	\begin{tikzpicture}
		...
		% Draw text
		\pgfmathsetmacro{\mylen}{5}
		\node[below=\mylen pt of T11] {$A^{[1]}$};
		\node[below=\mylen pt of T21] {$A^{[2]}$};
		\node[below=\mylen pt of T31] {$\Lambda^{[3]}$};
		\node[below=\mylen pt of T41] {$B^{[4]}$};
		\node[below=\mylen pt of T51] {$B^{[5]}$};
	\end{tikzpicture}
\end{document}
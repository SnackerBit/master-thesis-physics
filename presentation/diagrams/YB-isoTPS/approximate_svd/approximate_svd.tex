...
\begin{document}
	...
	\begin{tikzpicture}
		...
		\node[] () at (\xOffset+\tensorDistance+\svdDistance, \smallTensorWidth/2+\defaultTextOffsetLarge) {$\tilde{S}$};
		\node[tensorPhysicalSmall, fill=white] (Y) at (\xOffset+\tensorDistance+2*\svdDistance, 0) {};
		\node[] () at (\xOffset+\tensorDistance+2*\svdDistance, \smallTensorWidth/2+\defaultTextOffsetLarge) {$\tilde{Y}^*$};
		
		\begin{pgfonlayer}{bg}
			\draw[auxillaryLegSmall] (Q) -- (A);
			\draw[auxillaryLegSmall] (A) -- (S);
			\draw[auxillaryLegSmall] (Y) -- (S);
			\draw[virtualLegSmall, transform canvas={xshift={\defaultLegSeperationSmall*sin(0)/2}, yshift={-\defaultLegSeperationSmall*cos(0)/2}}] (Y)+(\tensorDistance-\smallTensorWidth/2, 0) -- (Y);
			\draw[specialLeg, transform canvas={xshift={-\defaultLegSeperationSmall*sin(0)/2}, yshift={\defaultLegSeperationSmall*cos(0)/2}}] (Y)+(\tensorDistance-\smallTensorWidth/2, 0) -- (Y);
			\draw[virtualLegSmall, transform canvas={xshift={\defaultLegSeperationSmall*sin(0)/2}, yshift={-\defaultLegSeperationSmall*cos(0)/2}}] (Q)+(-\tensorDistance+\smallTensorWidth/2, 0) -- (Q);
			\draw[specialLeg, transform canvas={xshift={-\defaultLegSeperationSmall*sin(0)/2}, yshift={\defaultLegSeperationSmall*cos(0)/2}}] (Q)+(-\tensorDistance+\smallTensorWidth/2, 0) -- (Q);
		\end{pgfonlayer}
		...
		\node[] () at (\xOffset, \smallTensorWidth/2+\defaultTextOffsetLarge) {$\tilde{X}$};
		\node[tensorPhysicalSmall, fill=white] (S) at (\xOffset+\tensorDistance, 0) {};
		\node[] () at (\xOffset+\tensorDistance, \smallTensorWidth/2+\defaultTextOffsetLarge) {$\tilde{S}$};
		\node[tensorPhysicalSmall, fill=white] (Y) at (\xOffset+2*\tensorDistance, 0) {};
		\node[] () at (\xOffset+2*\tensorDistance, \smallTensorWidth/2+\defaultTextOffsetLarge) {$\tilde{Y}^*$};
		
		\begin{pgfonlayer}{bg}
			\draw[auxillaryLegSmall] (X) -- (S);
			\draw[auxillaryLegSmall] (Y) -- (S);
			\draw[virtualLegSmall, transform canvas={xshift={\defaultLegSeperationSmall*sin(0)/2}, yshift={-\defaultLegSeperationSmall*cos(0)/2}}] (Y)+(\tensorDistance-\smallTensorWidth/2, 0) -- (Y);
			\draw[specialLeg, transform canvas={xshift={-\defaultLegSeperationSmall*sin(0)/2}, yshift={\defaultLegSeperationSmall*cos(0)/2}}] (Y)+(\tensorDistance-\smallTensorWidth/2, 0) -- (Y);

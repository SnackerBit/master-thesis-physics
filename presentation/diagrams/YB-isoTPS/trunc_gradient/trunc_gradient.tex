...
\begin{document}
	...
	\def\braceOffsetX{10pt}
	\def\braceOffsetY{2}
	\def\braceOffsetXRight{-10pt}
	\def\braceOffsetYDown{15pt}
	\begin{tikzpicture}
		
		%\node[] () at (26.6pt, 0) {$\large\xi^\text{trunc/Rényi}_{i,j,k,l}\coloneqq\hspace{200pt},$};
		\node[] () at (\equationDistance/2+\tensorDistance+\openLegLength, 0) {$-$};
		
		\draw [black, thick] (-\tensorDistance-2*\openLegLength+\braceOffsetX, -\verticalDistance/2-\unitaryDist-1.5*\smallTensorWidth-\braceOffsetY-\braceOffsetYDown) to [square left brace] (-\tensorDistance-2*\openLegLength+\braceOffsetX, \verticalDistance/2+\unitaryDist+1.5*\smallTensorWidth+\braceOffsetY);
		\draw [black, thick] (\equationDistance+\tensorDistance+4*\openLegLength+2*\tensorDistance+\braceOffsetXRight, -\verticalDistance/2-\unitaryDist-1.5*\smallTensorWidth-\braceOffsetY-\braceOffsetYDown) to [square right brace] (\equationDistance+\tensorDistance+4*\openLegLength+2*\tensorDistance+\braceOffsetXRight, \verticalDistance/2+\unitaryDist+1.5*\smallTensorWidth+\braceOffsetY);
		...
		\node[] () at (0, \verticalDistance/2+\smallTensorWidth/2+\defaultTextOffsetLarge) {$S$};
		...
		\node[] () at (\xPos, -\verticalDistance/2-\smallTensorWidth/2-\defaultTextOffsetLarge-\unitaryDist-\smallTensorWidth) {$S$};

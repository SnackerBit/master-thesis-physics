...
def split_psi(Psi, dL, dR, truncation_par = {'chi_max':32, 'p_trunc':1e-6}, verbose=0, max_iter=50, A=None):
	d,mL,mR = Psi.shape
	
	if np.any(A):
		chi = np.min([truncation_par['chi_max'],dR*mR,dL*mL])
		A, Psi_p = split_Psi_helper(Psi,dL,dR,chi,max_iter,verbose, A_hint = A, S2_weight = 1.0)
		trunc_leg = np.linalg.norm(Psi - np.tensordot(A, Psi_p, axes = [[1, 2], [1, 2]]))
	else:	
		chi = 1
		trunc_leg = 1
		while trunc_leg > 1e-16 and chi <= truncation_par['chi_max'] and chi <= np.min([dR*mR,dL*mL]):
			A, Psi_p = split_Psi_helper(Psi,dL,dR,chi,max_iter,verbose=0, A_hint = A,S2_weight = 1.0)
			trunc_leg = np.linalg.norm(Psi - np.tensordot(A, Psi_p, axes = [[1, 2], [1, 2]]))
			chi+=1
...
def split_Psi_helper(Psi, dL, dR, m, N_split, verbose=0, S2_weight = 0., A_hint = None):
	...
	def find_A(Psi, A, Lambda, AdPsi = None):
		...
		if AdPsi is None:
			...
			X, y, Z = np.linalg.svd(dS.reshape( [d, dL*dR]), full_matrices=False)
			...
			try:
				X, y, Z = np.linalg.svd(dS.reshape( [dL*dR, dL*dR]), full_matrices=False)
			except np.linalg.LinAlgError:
				X, y, Z = svd(dS.reshape( [dL*dR, dL*dR]), full_matrices=False)
	...
	def find_Lambda(Psi, A, truncation_par):
		...
		Lambda = np.dot( X[:, :chi]*(y[:chi]/norm), Z[:chi, :]).reshape( [mL, dL, dR, mR])
	...
	if A_hint is None:
		...
		try:
			X, y, Z = np.linalg.svd(Psi.reshape([d, mL*mR]))			
		except np.linalg.LinAlgError:
	...
	while delta_ne > 10e-12 and cnt < N_split:
		...
		ne = 0.5*np.linalg.norm(np.tensordot(AdPsi - Lambda.transpose([1, 2, 0, 3]), Lambda.conj(), axes = [[2, 3], [0, 3]]))
		delta_ne = np.abs(ne-ne_old)/ne

		ne_old = ne
		if verbose == 2:
			print i,
			print p_trunc, 1-np.sum(y), 0.5*np.linalg.norm(np.tensordot(AdPsi - Lambda.transpose([1, 2, 0, 3]), Lambda.conj(), axes = [[2, 3], [0, 3]])), np.linalg.norm(Aold - A)

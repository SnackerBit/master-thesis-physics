...
#         0->----<- 2
#
def split_psi(psi, dL, dR, truncation_par = {'chi_max':256, 'p_trunc':1e-6}, eps = 1e-8,  max_iter = 300, X = None, verbose = 0, final_A_iter = False, constrain=False):
	...
	lam = np.ascontiguousarray(lam.reshape((dL, dR, mL, mR)).transpose([0, 2, 1, 3]).reshape((dL*mL, dR*mR)))
	...
	while m < max_iter and go:
		...
		rhoL = syrk(1., lam.T, trans=1) #seems faste for some reason but...
		#rhoL = syrk(1., lamF) # = dot(lam, lam.T), will be herk for complex
		#rhoL = np.dot(lam, lam.T)
		
		### Code below would implement 1+nth  renyi measure
		# n = 0.5
		# p, X = np.linalg.eigh(rhoL, UPLO = 'U')
		# p[p<0] = 0.
		# dS = np.dot(X*p**n, X.T.conj()).reshape((dR, mR, dR, mR))
		# S2s.append(np.sum(p**(1+n)))
		
		
		#dS = np.dot(rhoL, lam)
		dS = symm(1., rhoL, lam.T, side = 1).T
		#dS = symm(1., rhoL, lamF, side = 0) #will be hemm in complex
		dS = np.ascontiguousarray(dS.reshape((dL, mL, dR, mR)).transpose([0, 2, 1, 3]).reshape((dL*dR, mL*mR)))  # dS = rhoL.lam --> dL x dR, mL x mR
		...
		S2s.append(np.sum(Y)) #This is to  close approximation Tr(rho_L^2) as A_n \sim A_{n+1}
		#S2s.append( np.linalg.norm(rhoL)**2)
	...
	if trans:
		...
		A = A.transpose([0, 2, 1])
		lam = lam.T
		psi = psi.reshape((-1, mR, mL)).transpose([0, 2, 1]).reshape((-1, mL*mR))
	...
	d_error = 2 - 2*np.sqrt(cum[-1]) #This is the error at eta = infinity, induced by finite dL*dR < d
	target_error = np.max( [ truncation_par['p_trunc'], 0.5*d_error]) # We select eta to induce an error of the same order
	eta = np.min([ np.count_nonzero((1 - cum) > 2*target_error )+ 1, truncation_par['chi_max']])
	...
	if final_A_iter:
		lam_trunc = np.dot(S, B).reshape((dL, mL, dR, mR)).transpose([0, 2, 1, 3]).reshape((dL*dR, mL*mR))
	...
	if verbose:
		print "			Disentangle Psi Evaluations:", m, ". Error at eta-",eta, " :", 2 - 2*nrm
		if verbose > 1:
	...
	#if (2 - 2*nrm) > 1e-10:
	#	print   (2 - 2*opt), d_error, (2 - 2*nrm), (2 - 2*opt)/(2 - 2*nrm)
	info = {'num_iter':m, 'opt_error': 2 - 2*opt, 'd_error':d_error, 'error':2 - 2*nrm, 's_AdPsi':s, 's_Lambda':s[:eta]/nrm}
...
def autod_split_psi(psi, truncation_par, verbose = 0):
	...
	for dR in range(1, 1+max):
		
		dL = max/dR
		
		a, S, B, info = split_psi(psi, dL, dR, truncation_par = {'chi_max':eta0_max, 'p_trunc':truncation_par['p_trunc']}, verbose=0, X = X)
		s = info['s_AdPsi']

		S1, S2, etaFOM = np.sum(s**2), np.sum(s**4), 1 - np.sum(s[:eta0_max]**2)
...
def split_landscape(psi, chiT_max, eta_max, verbose = 0):
	...
	for dL in range(1, dMax+1):
		for dR in range(1, dMax+1):
			...
			a, S, B, info = split_psi(psi, dL, dR, {'chi_max':eta_max, 'p_trunc':0.}, verbose=0)
			if verbose:
				print dL, dR, ' | ',
			s = info['s_AdPsi']
	...
	if verbose:
		print "Best dL, dR = , S2 = ", dL, dR, -np.log(S2s[dL-1, dR-1]), -np.log(S2s[1, 1])
	...
	if verbose > 1:
		...
		plt.imshow( 1 - S1s, interpolation='nearest', cmap = plt.cm.jet, vmin = vmin, vmax = vmax, norm = LogNorm(vmin=vmin, vmax=vmax), extent = [0.5, dMax+0.5, dMax + 0.5, 0.5])
		...
		plt.imshow( -np.log(S2s), interpolation='nearest', cmap = plt.cm.jet, vmin = vmin, vmax = vmax, extent = [0.5, dMax+0.5, dMax + 0.5, 0.5])
		plt.ylabel(r'$\chi_V$', fontsize=16)
		plt.xlabel(r'$\chi_H$', fontsize=16)
		plt.colorbar(orientation='horizontal')
		plt.title(r'$\log(S_2)$')
		...
		plt.imshow( etaFOM, interpolation='nearest', cmap = plt.cm.jet, vmin = vmin, vmax = vmax, norm = LogNorm(vmin=vmin, vmax=vmax), extent = [0.5, dMax+0.5, dMax + 0.5, 0.5])
		plt.ylabel(r'$\chi_V$', fontsize=16)
		plt.xlabel(r'$\chi_H$', fontsize=16)
		try:
			plt.colorbar(orientation='horizontal')
...
def split_quad(psi, dL, dR, truncation_par = {'chi_max':256, 'p_trunc':1e-6}, eps = 1e-6,  max_iter = 20, X = None, verbose = 0, constrain=True, dense_form=False, mode = 'S2'):
	...
	m = 0
	go = True
	#print "IN2", lam.reshape(-1)
	#Iterate via polar-decomposition of steepest descent on S2L*S2R

	"""
	rhoC = np.tensordot(lam, lam, axes = [[0, 2], [0, 2]])
	p, x = np.linalg.eigh(rhoC)
	p[p<1e-14] = 1e-14
	p = p/np.sum(p)
	#lp = -np.nan_to_num(np.log(p))+1.
	lp = -p
	dSC = np.dot(x*lp, x.T)
	#S2C = np.exp(-np.dot(lp, p))
	S2C = np.sum(p**2)
	dSC = np.tensordot(dSC, lam, axes = [[1], [1]]).reshape((mC, dL, mL, dR, mR)).transpose([1, 3, 2, 0, 4]).reshape((dL*dR, mL*mC*mR))
	"""
	while m < max_iter and go:
		...
		dSL = np.dot(dSL, l).reshape((dL, mL, mC, dR, mR)).transpose([0, 3, 1, 2, 4]).reshape((dL*dR, mL*mC*mR))
		...
		dSR = np.dot(dSR, l.T).reshape((dR, mR, dL, mL, mC)).transpose([2, 0, 3, 4, 1]).reshape((dL*dR, mL*mC*mR))
		...
		lam = np.dot(A.T, psi)
		lam = np.ascontiguousarray(lam.reshape((dL, dR, mL, mC,  mR)).transpose([0, 2, 3, 1, 4]))
...
def autod_split_quad(psi, truncation_par, verbose = 0, dense_form=False):
	...
	for dR in range(1, np.min([max, int(np.ceil(np.sqrt(max)))])+1):
		...
		a, S, B, info = split_quad(psi, dL, dR, truncation_par = {'chi_max':eta0_max, 'p_trunc':truncation_par['p_trunc']}, verbose=0, dense_form = dense_form)
		s = info['s_AdPsi']

		S1, S2, etaFOM = info['S1'], info['S2LR'], 1 - np.sum(s[:eta0_max]**2)

...
\end{equation}
under the constraints \eqref{eq:YB_isoTPS_YB_move_constraints}. Because the only tensors that are changed by the YB move are $W_1$, $W_2$ and $T$ and the three tensors make up a subregion of the full network with only incoming arrows, we can use the isometry condition to reduce the computation of the overlap $\braket{\Psi|\Psi^\prime}$ to a contraction of only six tensors as shown in Figure \figref{fig:YB_move_iterate_polar_overlap}.\par
\begin{figure}
	\centering

...
\begin{document}
	\begin{tikzpicture}
		...
		\node[tensorOperatorSmall, minimum width=\tensorDistanceMPS+\tensorWidthMPS, minimum height=\tensorWidthMPS] (op1) at (\tensorDistanceMPS/2, -\opOffset) {$\hat{U}^{[j,j+1]}$};
		...
		\node[] at (0, \tensorWidthMPS/2+1.5*\defaultTextOffset) {$\Lambda^{[j]}$};
		\node[] at (\tensorDistanceMPS, \tensorWidthMPS/2+1.5*\defaultTextOffset) {$B^{[j+1]}$};
		...
		\node[] at ({\blobPosX}, \blobPosY+\tensorWidthMPS/2+1.5*\defaultTextOffset) {$\theta^{[j, j+1]}$};
	
		\node[] () at ({\blobPosX+\blobhalfwidth+\tensorDistanceMPS-\tensorWidthMPS+\equationDistance/2}, -\opOffset+\tensorWidthMPS/2+\defaultTextOffset) {$\approx$};
	...
	\node[] at ({\AfinalposX}, \AfinalposY+\tensorWidthMPS/2+1.5*\defaultTextOffset) {$A^{[j]}$};
	\node[] at ({\AfinalposX+\tensorDistanceMPS}, \AfinalposY+\tensorWidthMPS/2+1.5*\defaultTextOffset) {$\Lambda^{[j+1]}$};
	\end{tikzpicture}
\end{document}
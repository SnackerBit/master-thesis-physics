...
\begin{document}
	...
	\begin{tikzpicture}
		...
		\node[tensorOperatorSmall, minimum width=\thetaWidth, minimum height=\smallTensorWidth, fill=generalTensorColor, inner sep=0] (dXSY) at (0, \verticalDistance/2) {};
		\node[] () at (0, \verticalDistance/2+\smallTensorWidth/2+\defaultTextOffsetLarge) {$\text{d}(XS^{2\alpha-1}Y^\dagger)$};
		...
		\begin{pgfonlayer}{bg}
			...
			\draw[virtualLegSmall] (-\tensorDistance, \verticalDistance/2-\smallTensorWidth/2-\verticalOpenLegLength) -- ++(0, \verticalOpenLegLength);
		...
		\node[tensorOperatorSmall, minimum width=\thetaWidth, minimum height=\smallTensorWidth, fill=generalTensorColor, inner sep=0] (dXSY) at (\xPos, -\verticalDistance/2-\unitaryDist-\smallTensorWidth) {$\text{d}(XSY^\dagger)^*$};

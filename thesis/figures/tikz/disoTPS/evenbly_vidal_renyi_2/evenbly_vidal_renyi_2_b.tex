...
\begin{document}
	...
	\def\EtensorHeight{1.3*\smallTensorWidth}
	\begin{tikzpicture}
		
		\def\yPos{-\verticalOffsetUnitaries-2*\thetaUdist-2*\smallTensorWidth-2*\unitaryWidth-\verticalOffsetThetaTensors}
		\node[] () at (-\unitaryHeight/2-\verticalLegDistance*4-\equationDistance/2, {(\yPos)/2}) {$=$};
		\node[tensorOperatorSmall, minimum width=\unitaryHeight, minimum height=\EtensorHeight, fill=white, inner sep=0] (E) at (-\unitaryHeight/2-\verticalLegDistance*4-\equationDistance-\unitaryHeight/2, {(\yPos)/2}) {$E$};
		\begin{pgfonlayer}{bg}
			\draw[virtualLegSmallWithoutArrows, transform canvas={xshift={\defaultLegSeperationLarge*sin(90)/2}, yshift={-\defaultLegSeperationLarge*cos(90)/2}}] (E) -- ++(0, \EtensorHeight/2+\thetaUdist);
			\draw[virtualLegSmallWithoutArrows, transform canvas={xshift={-\defaultLegSeperationLarge*sin(90)/2}, yshift={\defaultLegSeperationLarge*cos(90)/2}}] (E) -- ++(0, \EtensorHeight/2+\thetaUdist);
		...
		\node[tensorOperatorSmall, minimum width=\unitaryHeight, minimum height=0.7*\unitaryWidth, color=white] (U1) at (0, \verticalOffsetUnitaries/2+\unitaryWidth/2) {};

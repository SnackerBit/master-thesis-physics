...
\begin{document}
	...
	\begin{tikzpicture}
		...
		\draw [black, thick] ({-\diagonalDistance+(-2*\smallestDistance-\diagonalDistance)*\openAuxillaryLegMultiplier+\braceOffsetX}, -\diagonalDistance-\braketDistance-\diagonalDistance*\outerOpenVirtualLegMultiplier-\braceOffsetY) to [square left brace] ({-\diagonalDistance+(-2*\smallestDistance-\diagonalDistance)*\openAuxillaryLegMultiplier+\braceOffsetX}, \diagonalDistance+\diagonalDistance*\outerOpenVirtualLegMultiplier+\braceOffsetY);
		
		\draw [black, thick] ({4*\smallestDistance+\diagonalDistance+(2*\smallestDistance+\diagonalDistance)*\openAuxillaryLegMultiplier-\braceOffsetX}, -\diagonalDistance-\braketDistance-\diagonalDistance*\outerOpenVirtualLegMultiplier-\braceOffsetY) to [square right brace] ({4*\smallestDistance+\diagonalDistance+(2*\smallestDistance+\diagonalDistance)*\openAuxillaryLegMultiplier-\braceOffsetX}, \diagonalDistance+\diagonalDistance*\outerOpenVirtualLegMultiplier+\braceOffsetY);

		\node[] () at ({-\diagonalDistance+(-2*\smallestDistance-\diagonalDistance)*\openAuxillaryLegMultiplier-\braceOffsetX-59pt}, -\braketDistance/2) {$\Re\langle\Psi|\hat{U}^{[i,j]}(\Delta t)|\Psi^\prime\rangle=\Re$};
		...
		% one-site operator
		\node[tensorOperatorSmall, minimum width={4*\smallestDistance+\smallTensorWidth}, minimum height=\operatorHeight, inner sep=0] (op) at (2*\smallestDistance, -\braketDistance/2) {$\hat{U}^{[i,j]}$};
		
		% Second layer
		\node[tensorPhysicalSmall] (T1cc) at (0, -\braketDistance) {};	
		\node[] () at ({-sin(45)*(\smallTensorWidth/2+\defaultTextOffsetLarge)}, {-\braketDistance+cos(45)*(\smallTensorWidth/2+\defaultTextOffsetLarge)}) {$T_i^{\prime*}$};
		\node[tensorPhysicalSmall] (T2cc) at (4*\smallestDistance, -\braketDistance) {};	
		\node[] () at (4*\smallestDistance, -\smallTensorWidth/2-\defaultTextOffsetLarge-\braketDistance) {$T_j^{\prime*}$};
		\node[tensorAuxillarySmall] (W1cc) at (-\diagonalDistance, -\diagonalDistance-\braketDistance) {};
		\node[] () at (-\diagonalDistance, -\diagonalDistance+\smallTensorWidth/2+\defaultTextOffsetLarge-\braketDistance) {$W_1^{\prime*}$};
		\node[tensorOrthoCenterSmall] (W2cc) at (2*\smallestDistance, -\braketDistance) {};
		\node[] () at (2*\smallestDistance, \smallTensorWidth/2+\defaultTextOffsetLarge-\braketDistance) {$W_2^{\prime*}$};
		\node[tensorAuxillarySmall] (W3cc) at (4*\smallestDistance+\diagonalDistance, \diagonalDistance-\braketDistance) {};
		\node[] () at (4*\smallestDistance+\diagonalDistance, \diagonalDistance+\smallTensorWidth/2+\defaultTextOffsetLarge-\braketDistance) {$W_3^{\prime*}$};

...
\begin{document}
	...
	\begin{tikzpicture}
		...
		\node[tensorOrthoCenterSmall, fill=white, draw=white] (R) at (\tensorDistance/2, -\verticalDistance/2) {};
		...
		\node[] () at (\xOffset, \smallTensorWidth/2+\defaultTextOffsetLarge) {$\Lambda$};
		\node[tensorPhysicalSmall, fill=white] (R) at (\xOffset+\tensorDistance, 0) {};
		\node[] () at (\xOffset+\tensorDistance, \smallTensorWidth/2+\defaultTextOffsetLarge) {$R^\prime$};
		
		\begin{pgfonlayer}{bg}
			\draw[auxillaryLegSmall] (R) -- (Q);
			\draw[auxillaryLegSmall] (Q)+(-\tensorDistance+\smallTensorWidth/2, 0) -- (Q);

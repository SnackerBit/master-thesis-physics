\begin{figure}
	...
	\begin{tikzpicture}[scale=1, trim axis left, trim axis right]
		...
		\end{axis}
	\end{tikzpicture}
	\caption{We plot the Rényi-2 entropy against the number of iterations $N_\text{iter}$ of the Evenbly-Vidal disentangling algorithm. The environment of tensors on which the disentangling is performed is the same as in figure \protect\figref{fig:disoTPS_tripartite_decomposition_iterate_polar}. The Evenbly-Vidal disentangling algorithm converges quickly after only $N_\text{iter}\approx20$ iterations.}
